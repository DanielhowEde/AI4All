...
import { NANO_UNITS } from '../../fixedPoint';

/** Convert bigint nanounits to token number for JSON responses */
function toTokenDisplay(nano: bigint): number {
  return Number(nano) / Number(NANO_UNITS);
}
...
export function createAccountsRouter(state: ApiState): Router {
  ...
  router.get('/:accountId/balance', (req: Request, res: Response) => {
    ...
    if (!state.balanceLedger) {
      ...
      res.status(200).json({
        ...
        totalEarned: 0,
        lastRewardDay: null,
        note: 'Balance tracking requires persistent backend',
      });
      return;
    }

    const row = state.balanceLedger.getBalance(accountId);
  ...
  router.get('/:accountId/history', (req: Request, res: Response) => {
    ...
    const rows = state.balanceLedger.getHistory(accountId, limit);
  ...
  router.get('/leaderboard', (req: Request, res: Response) => {
    ...
    const rows = state.balanceLedger.getLeaderboard(limit);
  ...
  router.get('/supply', (_req: Request, res: Response) => {
    ...
    const supplyMicro = state.balanceLedger.getTotalSupply();
